            ...
            if (data && data.id) {
              ...
              try {
                if (event === 'patient.upserted' && data.name) {
                  ...
                  // Delete orphaned appointments to maintain Data Consistency (Saga Pattern)
                  const appointmentRepo = dataSource.getRepository(Appointment);
                  await appointmentRepo.delete({ patientId: data.id as number });
                  logger.info('[Saga] Orphaned appointments deleted for patient', { patientId: data.id });
                }
